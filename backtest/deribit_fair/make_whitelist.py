...
选择逻辑(镜像 bot 的 decide() 可交易域 + 校准结论 "只卖上行尾 2-7c"):
  ...
  - BTC/ETH(有 Deribit 锚): edge = sell_px − fair_hi,要求 ≥ 0.8c,按 edge 排名;
        ...
        for ev in evs or []:
            for m in ev.get("markets", []):
                ...
                out.append({"slug": m.get("slug"), "coin": coin, "ask": ask, "sell": px,
                            "fair_hi": round(px / ratio, 4), "edge": round(px * (1 - 1 / ratio), 4),
                            "anchor": f"hist{ratio}x", "days": round(days_left(T), 2)})
    return out

def main():
    n = int(sys.argv[1]) if len(sys.argv) > 1 else 12
    rows = sorted(anchored_rows() + unanchored_rows(), key=lambda r: -r["edge"])
    seen, top = set(), []
    for r in rows:
        if r["slug"] in seen:
            continue
    ...
    for r in top:
        print(f"  {r['slug']:55} {r['coin']:4} ask={r['ask']:.3f} sell={r['sell']:.3f} "
              f"fair≤{r['fair_hi']:.4f} edge={r['edge']:+.4f} d={r['days']:.1f} [{r['anchor']}]",
              file=sys.stderr)
