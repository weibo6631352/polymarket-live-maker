...
print("SETTLE_MS  n_trades  net_mean/sh   win%   t-stat   total$(5sh)   verdict")
for s in SETTLES:
    tr = replay(s)
    if not tr:
    ...
    tot = sum(tr) * 5
    print("  %4d    %4d   %+.4f      %3d%%   %5.2f   %+7.2f    %s" % (
        s, n, mean, 100 * sum(1 for x in tr if x > 0) // n, mean / se if se else 0, tot,
        "+EV" if mean > 0 else "NEGATIVE"))
